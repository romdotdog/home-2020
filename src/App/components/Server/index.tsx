...
const Join = styled.button`
	...
	:focus {
		box-shadow: rgba(32, 134, 55, 0.5) 0 0 0 0.2rem;
		outline: 0;
	}
...
const Server: React.FC<ServerProps> = ({ id, name, icon, compact }) => {
	...
	React.useEffect(() => {
		;(async () => {
			...
			const cache = (storage &&
				JSON.parse(storage)) as DiscordWidgetWithAccess | null

			if (cache && cache.access > now - 7200 * 1000) {
				// 7200 is 2 hours, so cache is only effective for two hours.
				newWidget = cache
			} else {
				newWidget = await (
					await fetch(`https://edge.rom.dog/discord/${id}`)
				).json()
	...
	return (
		<>
			<Main>
				...
				<Info>{name}</Info>
				<Join
					className={widget !== null ? "visible" : ""}
					onClick={
						widget !== null
							? () => openInvite(widget.instant_invite, name)
							: undefined
					}
				>
					Join
